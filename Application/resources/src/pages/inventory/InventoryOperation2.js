  ...
  return (
    <Modal title={mode === 'edit' ? 'Edit Item' : 'Add New Item'} open={visible} onCancel={onCancel} onOk={handleSubmit} confirmLoading={duplicateChecking} width={800}>
      <Form form={form} layout="vertical">
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item name="ItemCode" label="Item Code" rules={[{ validator: validateItemCode }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
              <Input
          ...
          <Col span={12}>
            <Form.Item name="Description" label="Description" rules={[{ validator: validateDescription }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
              <Input
        ...
        <Row gutter={16}>
          <Col span={8}>
            <Form.Item name="DutyRate" label="Duty Rate" rules={[{ validator: validateNumeric('Duty Rate') }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
              <InputNumber
                style={{ width: '100%' }} min={0} />
            </Form.Item>
          </Col>
          <Col span={8}>
            <Form.Item name="Price" label="Price" rules={[{ validator: validateNumeric('Price') }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
              <InputNumber style={{ width: '100%' }} min={0} />
            </Form.Item>
          </Col>
          <Col span={8}>
            <Form.Item name="BalQty" label="Balance Qty" rules={[{ validator: validateNumeric('Balance Qty') }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
        ...
        <Row gutter={16}>
          <Col span={8}>
            <Form.Item name="BaseUOM" label="Base UOM" rules={[{ validator: validateBaseUOM }]} validateTrigger={['onChange', 'onBlur']} hasFeedback>
...
const InventoryOperation = () => {
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        // Separately fetch ALL items for statistics
        const statsResponse = await api.get('/items', {
          params: {
            per_page: 9999,
            search: searchTerm,
  ...
  // Update the third useEffect (around line 554)
  useEffect(() => {
    if (stockSettings) {
      fetchItems({
  ...
  }, [stockSettings, fetchItems, pagination]);
